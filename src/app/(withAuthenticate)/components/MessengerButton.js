...
const MessengerButton = () => {
    ...
    useEffect(() => {
        if (isOpen) api.GET_MESSAGE_CHANNEL({ search: input }).then(res => {
            setMessageChannels(res.data)
        })
    }, [isOpen, input])
    return (
        <div>
            <div onClick={togglePopUp} className="avatar" style={{ height: 40, width: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isOpen ? 'var(--highlight-bg)' : 'var(--gray)' }}>
                <svg viewBox="0 0 28 28" fill={isOpen ? 'var(--accent)' : ''} height={20} width={20}><path d="M14 2.042c6.76 0 12 4.952 12 11.64S20.76 25.322 14 25.322a13.091 13.091 0 0 1-3.474-.461.956 .956 0 0 0-.641.047L7.5 25.959a.961.961 0 0 1-1.348-.849l-.065-2.134a.957.957 0 0 0-.322-.684A11.389 11.389 0 0 1 2 13.682C2 6.994 7.24 2.042 14 2.042ZM6.794 17.086a.57.57 0 0 0 .827.758l3.786-2.874a.722.722 0 0 1 .868 0l2.8 2.1a1.8 1.8 0 0 0 2.6-.481l3.525-5.592a.57.57 0 0 0-.827-.758l-3.786 2.874a.722.722 0 0 1-.868 0l-2.8-2.1a1.8 1.8 0 0 0-2.6.481Z" /></svg>
            </div>
            <div role='dialog' className="message-channel avatar-popup-container" id='message-channel-popup' style={{ width: 345 }} >
                <div style={{ padding: '8px 12px 4px', fontSize: 24, fontWeight: 700 }}>Chats</div>
                <div style={{ padding: 10, position: 'sticky', top: -5, background: 'white', zIndex: 1 }}>
                    <input placeholder="Search messenger" value={input} onChange={e => setInput(e.target.value)} style={{ height: 36 }} />
                </div>
                {
...
const MessageChannelItem = ({ data, onClick }) => {
    ...
    return (
        <div style={{ height: 72, display: 'flex' }} className="setting-button" onClick={() => onClick(data)}>
            <img src={data.participants[0].avatar} style={{ minHeight: 56, minWidth: 56 }} className="avatar" />
            <div style={{ width: '100%' }}>
