...
const MessengerButton = () => {
  ...
  useEffect(() => {
    if (isOpen)
      api.GET_MESSAGE_CHANNEL({ search: input }).then((res) => {
    ...
    <div>
      <div
        ...
        style={{
          ...
          background: isOpen ? "var(--highlight-bg)" : "var(--gray)",
        }}
      >
        <svg
          viewBox="0 0 28 28"
          fill={isOpen ? "var(--accent)" : ""}
          height={20}
          width={20}
        >
      ...
      <div
        role="dialog"
        className="message-channel avatar-popup-container"
        id="message-channel-popup"
        style={{ width: 345 }}
      >
        <div style={{ padding: "8px 12px 4px", fontSize: 24, fontWeight: 700 }}>
          Chats
        </div>
        <div
          style={{
